...
namespace Galaga.GalagaStates {
    public class GameRunning : IGameState {
        ...
        public ISquadron SetSquad(int NumEnemies, float offset, bool randidx, int index){
        ...
        public void CreateSquad(float offset, bool rand){
            ...
            if(rand){
                var squad = SetSquad(numEnemies, offset, true, 0);
            }
            else{
                var squad = SetSquad(numEnemies, offset, false, 0);
            }
            squad.CreateEnemies(enemyStridesBlue, enemyStridesRed, offset);
        }

        public IMovementStrategy SetRandMovementStrategy(){
            List<IMovementStrategy> movementStrategies = new List<IMovementStrategy> {new Down(), 
                new ZigZagDown(), new SideLoop()};
                int index = rand.Next(movementStrategies.Count);
                movementStrategy = movementStrategies[index];
        ...
        private void IterateEnemies(){
            ...
            if (squad.Enemies.CountEntities() == 0){
                //change level
                level.AddPoint();
                //change movement strategy randomly
                movementStrategy = SetRandMovementStrategy();
                //change squadron shape create new wave
                CreateSquad(-0.002f - (level.score * 0.0002f), true);
            }
